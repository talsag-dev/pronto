        ...
        state: {
            ...
            keys: {
                ...
                set: async (dataToSet) => {
                        ...
                        for (const id in dataToSet[category as keyof SignalDataTypeMap]) {
                            ...
                            if (value) {
                             updates.push({
                                ...
                                // If value is null/undefined, we should probably delete it
                                // But Baileys usually just overwrites.
                                // For now, we'll treat deletion as a separate step or just nullify
                                await supabaseAdmin
                                    .from('whatsapp_sessions')
                                    .delete()
                    ...
                    // Bulk upsert not perfectly supported for updates in one go if we rely on loop
                    // But Supabase upsert works well.
                    if (updates.length > 0) {
                        const { error } = await supabaseAdmin
                            .from('whatsapp_sessions')
                            .upsert(updates, { onConflict: 'session_id,key' as any }); // Cast needed if types aren't perfectly aligned
